...
import com.example.android.popularmovies_stage2.Trailer;
...
public class NetworkTrailerUtils {
    public static final String LOG_TAG = MainActivity.class.getSimpleName();


    public NetworkTrailerUtils() {
    ...
    public static ArrayList<Trailer> networkReqForTrailers(String id, String apiKey) throws JSONException {
        Log.e(LOG_TAG, "onNetworkReq");
        String urlForTrailers = "http://api.themoviedb.org/3/movie/" + id + "/videos?api_key=" + apiKey;
        URL URL = createUrl(urlForTrailers);
        ...
        ArrayList<Trailer> trailerList = parseResponse(jsonResponse, true);
        return trailerList;
    }

    private static ArrayList<Trailer> parseResponse(String JSONResponse, Boolean isTrailer) {
        ...
        if (isTrailer == true) {
            try {
                ...
                for (int i = 1; i < resultsArray.length(); i++) {
                    ...
                    trailers.add(new Trailer(youtube_URL));
                }
